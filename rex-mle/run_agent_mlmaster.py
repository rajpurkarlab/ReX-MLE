...
def setup_agent_workspace(
    ...
    if agent_dir.exists():
        for item in agent_dir.iterdir():
            if item.name in ["Dockerfile", "__pycache__"]:
                continue
            ...
            if item.is_symlink():
                ...
                if not target_path.is_absolute():
                    target_path = (item.parent / target_path).resolve()
...
async def run_agent_in_conda_env(
    ...
    run_logger.info(f"Executing agent in conda environment 'ml-master'...")
    ...
    try:
        ...
        # Stream output asynchronously without assuming newline-delimited logs
        assert process.stdout is not None
        buffer = b""
        max_buffer = 128 * 1024  # prevent unbounded growth when no newline is emitted
        while True:
