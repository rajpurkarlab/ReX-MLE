#!/usr/bin/env python
"""
RD-Agent specific runner that works without Docker.
This script handles rdagent's unique data directory structure requirements.
"""
import argparse
import asyncio
import json
...
def setup_rdagent_workspace(
    competition: Competition,
    agent: Agent,
    run_dir: Path,
) -> Path:
    """
    Set up the workspace directory structure for rdagent.

    RDAgent has specific requirements:
    - Data must be at: DS_LOCAL_DATA_PATH/<competition_id>/files...
    - This differs from other agents that expect data files directly in DATA_DIR
    ...
    # Copy public data with rdagent-specific structure
    # RDAgent expects: DS_LOCAL_DATA_PATH/<competition_id>/files...
    logger.info(f"Setting up rdagent workspace data from {competition.public_dir}")
    data_dir = dirs["data"]

    # Create competition subdirectory (REQUIRED by rdagent)
    competition_data_dir = data_dir / competition.id
    competition_data_dir.mkdir(parents=True, exist_ok=True)

    # Create symlinks or copy data into competition subdirectory
    if competition.public_dir.exists():
        # Use symlinks to save space and time
        for item in competition.public_dir.iterdir():
            link_target = competition_data_dir / item.name
    ...
    # Copy grade.py and prepare.py from challenge directory to competition data directory
    # This allows RDAgent to find the grading script when looking in the data directory
    challenge_dir = Path(__file__).parent / "rexmle" / "challenges" / competition.id
    if challenge_dir.exists():
        for script_name in ["grade.py", "prepare.py"]:
            src = challenge_dir / script_name
            if src.exists():
                dst = competition_data_dir / script_name
                # Only copy if destination doesn't exist (to avoid permission errors with symlinks)
                if not dst.exists():
                    try:
                        shutil.copy2(src, dst)
                        logger.info(f"Copied {script_name} to competition data directory: {dst}")
                    except (OSError, PermissionError) as e:
                        # If copy fails due to permissions, try creating a symlink instead
                        try:
                            if dst.exists() or dst.is_symlink():
                                dst.unlink()
                            dst.symlink_to(src)
                            logger.info(f"Created symlink for {script_name}: {dst} -> {src}")
                        except OSError:
                            logger.warning(f"Could not copy or symlink {script_name}: {e}")
                else:
                    logger.debug(f"{script_name} already exists at {dst}")
    ...
    # Copy .env file from project root to agent directory
    project_root = Path(__file__).parent
    env_file = project_root / ".env"
    if env_file.exists():
        shutil.copy2(env_file, dirs["agent"] / ".env")
        logger.info(f"Copied .env file to agent directory")
    else:
...
async def run_rdagent_in_conda_env(
    workspace: Path,
    agent: Agent,
    competition: Competition,
    ...
    """
    Execute rdagent's start script in the conda environment.
    ...
    run_logger.info(f"Executing rdagent in conda environment 'rexagent'...")
...
async def worker(
    ...
    while True:
        ...
        try:
            # Set up workspace with rdagent-specific structure
            workspace = setup_rdagent_workspace(
                task.competition,
                task.agent,
                task.path_to_run,
            )
            ...
            success = await run_rdagent_in_conda_env(
                workspace,
                task.agent,
                task.competition,
                run_logger,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run RD-Agent on a set of competitions WITHOUT Docker."
    )
    parser.add_argument(
        "--agent-id",
        help="Agent ID of the agent to run (should be 'rdagent')",
        type=str,
        required=True,
    )
    parser.add_argument(
