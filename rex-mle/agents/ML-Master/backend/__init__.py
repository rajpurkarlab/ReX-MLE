...
from backend.call import gpt_query, r1_query
...
def determine_provider(model: str) -> str:
    ...
    if name.startswith("gemini"):
        return "gemini"
    logger.warning(
        "Falling back to OpenAI-compatible provider for unrecognized model name '%s'",
        model,
    )
    return "openai"
...
def query(
    ...
    **model_kwargs,
    ...
    if provider == "openai":
        from . import backend_openai as _backend
    elif provider == "anthropic":
        from . import backend_anthropic as _backend
    elif provider == "gemini":
        from . import backend_gemini as _backend
    else:
        raise ValueError(f"Unsupported provider: {provider}")
    query_func = _backend.query
