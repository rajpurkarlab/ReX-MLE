...
def _install_json_numpy_support():
    """Ensure json handles numpy/pandas types without crashing user code."""
    try:
    ...
    except Exception:  # pragma: no cover - pandas may not be available
        pd = None  # type: ignore

    if getattr(json, "_rexmle_numpy_patch", False):
        return
    ...
    json.dump = _dump  # type: ignore
    json.dumps = _dumps  # type: ignore
    json._rexmle_numpy_patch = True  # type: ignore[attr-defined]
...
class Interpreter:
    ...
    def _run_session(
        ...
        while True:
            ...
            # Commented out to avoid CPU affinity errors on systems that don't support it
