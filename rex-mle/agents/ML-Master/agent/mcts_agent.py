...
import math
import humanize
from backend import FunctionSpec, compile_prompt_to_md, gpt_query, query, r1_query
    ...
    json_schema={
        ...
        "properties": {
            "is_bug": {
                "type": "boolean",
                "description": (
                    "true if the output log shows that the execution failed (exception/traceback) "
                    "or the run did not successfully produce a usable submission CSV under `./submission/`. "
                    "Do NOT mark as a bug solely because the submission filename is `submission_<...>.csv` "
                    "instead of exactly `submission.csv`, as the runner may rename it."
                ),
            },
            "has_csv_submission": {
                "type": "boolean",
                "description": (
                    "true if the code saves the predictions on the test data as a CSV file in the `./submission/` "
                    "directory. Accept either the canonical filename `submission.csv` OR a per-run/per-node filename "
                    "like `submission_<...>.csv` (the runner may rename `submission.csv` automatically). "
                    "If no submission CSV is produced in `./submission/`, return false."
                ),
            },
            "summary": {
                "type": "string",
                ...
                " the submission CSV was not properly produced in `./submission/`."
...
def _extract_metric_from_term_out(term_out: str) -> tuple[float | None, bool | None]:
    """
    Best-effort metric extraction from raw execution output.

    This exists because the LLM judge (especially smaller/older models) can fail to
    output a numeric `metric` even when the program prints it. We bias toward the
    last "metric-looking" line near the end of the logs.
    """
    keyword_groups: list[tuple[list[str], bool]] = [
        (["loss", "rmse", "mae", "mse", "error"], True),  # lower is better
        (["dice", "auc", "accuracy", "acc", "f1", "score", "metric"], False),  # higher is better
    ]

    lines = term_out.splitlines()
    float_re = re.compile(r"[-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?")

    candidates: list[tuple[int, float, bool | None]] = []
    for idx, line in enumerate(lines):
        lower = line.lower()
        matched_lib = False
        lower_is_better: bool | None = None

        for keywords, kw_lower_is_better in keyword_groups:
            if any(k in lower for k in keywords):
                matched_lib = True
                # If the line matches multiple groups (e.g., "loss" and "score"),
                # prefer "loss-like" semantics when present.
                if lower_is_better is None or kw_lower_is_better is True:
                    lower_is_better = kw_lower_is_better
                break
        ...
        # Heuristic: use the last numeric token on the line.
        try:
            val = float(nums[-1])
        except ValueError:
            continue

        # Guardrail: reject obviously non-metric values (timestamps, counts, etc.).
        if not (math.isfinite(val) and abs(val) <= 1e6):
            continue

        candidates.append((idx, val, lower_is_better))

    if not candidates:
        return None, None

    _, metric, lib = candidates[-1]
    return metric, lib
...
class MCTSAgent:
    ...
    def _draft(self) -> MCTSNode:
        ...
        # Default prompt format works for models using standard chat completion schemas (e.g., GPT family).
        user_prompt = (
        ...
        if "qwen3" in self.acfg.code.model:
            ...
            prompt_complete = (
                f"<|im_start|>system\n{introduction}<|im_end|>\n"
                f"<|im_start|>user{user_prompt}<|im_end|><|im_start|>assistant\n"
                "<think>Okay! Now, I will focus my efforts on successfully completing this current task.\n"
                "Before completing this task, first of all, I need to analyze and understand the relevant dataset. "
                f"The information of the dataset is as follows: \n{self.data_preview}"
            )
        ...
        elif "o4-mini" in self.acfg.code.model or "o1-" in self.acfg.code.model:
            user_prompt = (
                f"\n# Task description\n{prompt['Task description']}\n\n"
                f"# Memory\nThe memory of previous solutions used to solve task is provided below:\n{prompt['Memory']}\n\n"
                f"{instructions}\n\n# Data Preview\n{self.data_preview}"
    ...
    def plan_and_code_query(self, prompt, retries=3) -> tuple[str, str]:
        ...
        model_name = (self.acfg.code.model or "").lower()
    ...
    def parse_exec_result(self, node: MCTSNode, exec_result: ExecutionResult) -> MCTSNode:
        try:
            ...
            if response.get("has_csv_submission") is False and has_csv_submission is True:
                logger.info(
                    f"Overriding judge has_csv_submission=False for node {node.id} because the file exists on disk."
                )
                response["has_csv_submission"] = True

            if (
                response["metric"] is None
                and node.exc_type is None
                and has_csv_submission is True
            ):
                extracted_metric, extracted_lower_is_better = _extract_metric_from_term_out(
                    "".join(node._term_out or [])
                )
                if extracted_metric is not None:
                    logger.info(
                        f"Recovered metric={extracted_metric} for node {node.id} from execution output (judge returned null)."
                    )
                    response["metric"] = extracted_metric
                    if extracted_lower_is_better is not None:
                        response["lower_is_better"] = extracted_lower_is_better

            if (
                response.get("is_bug") is True
                and node.exc_type is None
                and has_csv_submission is True
                and response["metric"] is not None
            ):
                logger.info(
                    f"Overriding judge is_bug=True for node {node.id} because execution succeeded and a metric was found."
                )
            ...
            # Prefer hard signals (exception + missing submission + missing metric) over the LLM's is_bug.
            # The judge can be overly conservative with some models and mark successful runs as buggy.
            node.is_buggy = (
                node.exc_type is not None
                or response["metric"] is None
                or has_csv_submission is False
            )
    ...
    def parse_exec_result_without_tool(self, node: MCTSNode, exec_result: ExecutionResult) -> MCTSNode:
        ...
        introduction = (
            ...
            "- has_csv_submission (boolean): This field indicates whether a submission CSV file was generated in the ./submission/ directory. Accept either submission.csv OR a renamed file like submission_<...>.csv (the runner may rename submission.csv automatically). If no submission CSV exists under ./submission/, set this to false.\n"
...
"""
    ...
    def select(self, node: MCTSNode):
        ...
        # Check if we're at root and all children are terminal or locked
        if self.is_root(node):
            available_children = [child for child in node.children if not child.lock]
            all_terminal = all(child.is_terminal for child in available_children) if available_children else True
            if all_terminal and len(available_children) > 0:
                logger.warning(f"All available children of root are terminal. Search cannot continue.")
                # Mark the root as terminal to signal search completion
        ...
        while node and not node.is_terminal:
            if not node.is_fully_expanded_with_expected(scfg=self.scfg):
                if node.is_buggy and node.is_debug_success is True:
                    next_node = self.uct_select(node)
    ...
    def uct_select(self, node: MCTSNode):
        if self.is_root(node):
            unlocked_children = [child for child in node.children if not child.lock]
            filtered_children = [child for child in unlocked_children if not child.is_terminal]
            logger.info(f"For node {node.id}, there are {len(node.children) - len(unlocked_children)}/{len(node.children)} is locked.")
            if unlocked_children and not filtered_children:
                # Nothing left to expand under root.
                if all(child.is_terminal for child in unlocked_children):
                    logger.warning("Root has no non-terminal unlocked children; marking root as terminal.")
                    node.is_terminal = True
                return node
            selected_node = node
            ...
            filtered_children = [child for child in node.children if not child.is_terminal]
            if node.children and not filtered_children and all(child.is_terminal for child in node.children):
                node.is_terminal = True
                return node
            if not filtered_children:
                return node
            return max(filtered_children, key=lambda child: child.uct_value(exploration_constant = self.get_C()))
            ...
            if self.best_node is None or self.best_node.metric < result_node.metric:
                ...
                if self.best_node is None or result_node.is_valid is True:
                    ...
                    with self.save_node_lock:
                        ...
                        # Copy entire submission directory for multi-file submissions
                        # (e.g., dentex, puma with predictions/*.json)
                        submission_dir = self.cfg.workspace_dir / "submission"
                        if submission_dir.exists():
                            # Copy all subdirectories and files except any CSV files
                            for item in submission_dir.iterdir():
                                if item.is_dir():
                                    dest = best_submission_dir / item.name
                                    if dest.exists():
                                        shutil.rmtree(dest)
                                    shutil.copytree(item, dest)
                                elif item.is_file() and not item.suffix == ".csv":
                                    # Copy other non-CSV files that might be needed
                                    shutil.copy(item, best_submission_dir / item.name)

                        with open(best_solution_dir / "solution.py", "w") as f:
                            f.write(result_node.code)
                        with open(best_submission_dir / "node_id.txt", "w") as f:
