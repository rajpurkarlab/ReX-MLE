...
def run():
    ...
    # Prevent duplicate logging when an upstream/root handler is also configured
    # (common when running under orchestration that wraps stdout).
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        while completed <= total_steps and not agent.virtual_root.is_terminal:
        ...
        logger.info(f"Total nodes explored: {completed}")
        for fut in futures:
            fut.cancel()
