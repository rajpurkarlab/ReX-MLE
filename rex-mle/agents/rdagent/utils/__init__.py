...
def filter_redundant_text(stdout: str) -> str:
    ...
    # Pre-shrink if stdout is too large to avoid tiktoken stack overflow
    # (tiktoken can crash with stack overflow on very large strings, e.g., 5MB+)
    MAX_STDOUT_SIZE = 1_000_000  # 1MB limit before attempting tokenization
    if len(filtered_stdout) > MAX_STDOUT_SIZE:
        logger.warning(f"Stdout too large ({len(filtered_stdout)} bytes), pre-shrinking to avoid tokenizer crash")
        filtered_stdout = _shrink_stdout_once(filtered_stdout)
    ...
    for _ in range(3):
        ...
        for __ in range(10):
            try:
                ...
                # can raise ValueError or PanicException (from Rust) on very large strings
