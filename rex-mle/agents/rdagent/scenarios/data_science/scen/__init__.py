...
class DataScienceScen(Scenario):
    ...
    def __init__(self, competition: str) -> None:
        ...
        # check_runtime_environment(get_ds_env())  # Patched for no-Docker
        # 1) prepare data
    ...
    def _get_description(self):
        ...
        if (fp := Path(f"{DS_RD_SETTING.local_data_path}/{self.competition}/description.md")).exists():
            ...
            logger.error(
                f"Cannot find '{self.competition}.json' or 'description.md' file in {DS_RD_SETTING.local_data_path}"
            )

        # Append grade.py if it exists in the same directory as description
        if description is not None:
            grade_py_path = Path(f"{DS_RD_SETTING.local_data_path}/{self.competition}/grade.py")
            if grade_py_path.exists():
                logger.info(f"{self.competition}/Found grade.py, appending to description.")
                description += "\n\n## GRADING SCRIPT\n\n```python\n"
                description += grade_py_path.read_text()
                description += "\n```\n"
            else:
                logger.warning(f"{self.competition}/grade.py not found at {grade_py_path}")
    ...
    def get_runtime_environment(self) -> str:
        ...
        # Determine conf_type based on whether we're using MLE data
        conf_type = "mlebench" if DS_RD_SETTING.if_using_mle_data else "kaggle"
        ds_env = get_ds_env(conf_type=conf_type)
