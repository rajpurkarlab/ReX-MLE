...
class MLETestEval(TestEvalBase):
    ...
    def __init__(self) -> None:
        super().__init__()
        # For LocalEnv (conda mode), use the actual path directly instead of /mle/data
        # Docker mode would use: extra_volumes={f"{DS_RD_SETTING.local_data_path}/zip_files": "/mle/data"}
        self.env = get_ds_env(conf_type="mlebench")
        self.env.prepare()
        # Determine the data directory path based on environment type
        from rdagent.components.coder.data_science.conf import DSCoderCoSTEERSettings
        import os
        from pathlib import Path
        if DSCoderCoSTEERSettings().env_type == "conda":
            # Use the actual MLEBench prepared data directory
            # First try DATA_DIR environment variable (set by run_agent_no_docker.py)
            # Fall back to looking for challenge_data relative to workspace
            data_dir = os.getenv("DATA_DIR")
            if data_dir is None or not Path(data_dir).exists():
                # Try relative path from current working directory
                fallback_dir = Path.cwd().parent / "challenge_data"
                if fallback_dir.exists():
                    data_dir = str(fallback_dir)
                else:
                    # Last resort: use environment variable or default
                    data_dir = os.getenv("rexmleBENCH_DATA_DIR", "../../challenge_data")
            self.mle_data_dir = data_dir
        else:
            # Docker mode
            self.mle_data_dir = "/mle/data"

    def eval(self, competition: str, workspace: FBWorkspace) -> str:
        workspace.execute(
            env=self.env,
            entry=f"mlebench grade-sample submission.csv {competition} --data-dir {self.mle_data_dir} 2>&1 | tee mle_score.txt",
    ...
    def valid(self, competition: str, workspace: FBWorkspace) -> tuple[str, int]:
        mle_check_code = (
            ...
            .replace("/mle/data", self.mle_data_dir)  # Replace hardcoded path with actual data dir
        )
