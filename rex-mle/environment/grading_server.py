...
CHALLENGE_ID = os.getenv("CHALLENGE_ID")  # This is populated for us at container runtime


def run_validation(submission: Path) -> str:
    new_registry = registry.set_data_dir(Path(PRIVATE_DATA_DIR))
    challenge = new_registry.get_challenge(CHALLENGE_ID)
    is_valid, message = validate_submission(submission, challenge)
    return message
