"""
Grading script for PUMA Track 1 Task 1 - Semantic Tissue Segmentation
...
def calculate_dice_per_class(tif1: np.ndarray, tif2: np.ndarray, image_shape: tuple, eps: float = 0.00001) -> Dict[str, float]:
    ...
    """
    # Ensure integer dtype for proper comparison
    tif1 = tif1.astype(np.uint8)
    tif2 = tif2.astype(np.uint8)
...
def grade(submission: pd.DataFrame, answers: pd.DataFrame, submission_dir: Optional[Path] = None, answers_dir: Optional[Path] = None) -> Dict[str, float]:
    ...
    for idx, row in submission.iterrows():
        ...
        try:
            ...
            # Handle multi-channel images by taking first channel if needed
    ...
    # Print summary
    print(f"\n=== PUMA Track 1 Task 1 (Tissue Segmentation) Results ===")
