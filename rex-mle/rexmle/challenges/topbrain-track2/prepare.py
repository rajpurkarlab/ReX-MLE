...
def prepare(raw: Path, public: Path, private: Path) -> None:
    ...
    sample_sub['predicted_mask_path'] = sample_sub['image_id'].apply(
        lambda x: f"predictions/topcow_mr_{x}.nii.gz"
    )
...
- Labels: `topcow_mr_{{pat_id}}.nii.gz` (multiclass segmentation with 42 vessel labels)
...
├── submission.csv
└── predictions/
    ├── topcow_mr_001.nii.gz
    ├── topcow_mr_002.nii.gz
    └── ...
```
