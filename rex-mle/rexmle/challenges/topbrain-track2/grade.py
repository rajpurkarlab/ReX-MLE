"""
Grading script for TopBrain Track 2 - MRA Multiclass Brain Vessel Segmentation
...
# ===========================
# Constants for MRA Track 2
# ===========================

# MRA label map (42 vessel classes + background)
...
# ===========================
# Metric 4: Hausdorff Distance 95% (HD95)
# ===========================
...
def hd95_single_label(gt: sitk.Image, pred: sitk.Image, label: int) -> float:
    ...
    surface_distances = get_surface_distance(edges_pred, edges_gt, "euclidean", spacing)
...
def detection_sideroad_labels(gt: sitk.Image, pred: sitk.Image) -> dict:
    ...
    for label in SIDEROAD_COMPONENT_LABELS_MR:
        ...
        detection_dict[str(label)] = {
            "label": MUL_CLASS_LABEL_MAP_MR[str(label)],
            "Detection": detection,
        }
...
def grade(submission: pd.DataFrame, answers: pd.DataFrame,
    ...
    """
    Grade TopBrain Track 2 (MRA) submission using comprehensive TopBrain metrics.
    ...
    # Load valid neighbors for MRA
    valid_neighbors_path = Path(__file__).parent / "valid_neighbors_mr_all.json"
