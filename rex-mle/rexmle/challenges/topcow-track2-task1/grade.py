"""
Grading script for TopCoW Track 2 Task 1 - MRA Multi-class Segmentation
...
def grade(submission: pd.DataFrame, answers: pd.DataFrame, submission_dir: Optional[Path] = None, answers_dir: Optional[Path] = None) -> dict:
    """
    Grade TopCoW Track 2 Task 1 submission using comprehensive TopCoW metrics.
    ...
    print(f"Anterior graph accuracy: {anterior_accuracy:.4f}")
    print(f"Posterior graph accuracy: {posterior_accuracy:.4f}")
    ...
    if leaderboard_path.exists():
        try:
            ...
            results_dict['overall'] = -(mean_dice + mean_cldice + f1_grp2 + anterior_accuracy + posterior_accuracy + anterior_topology + posterior_topology) / 7 + (mean_b0_error + mean_hd95)
    else:
        # Fall back to negative mean of metrics as overall score
        results_dict['overall'] = -(mean_dice + mean_cldice + f1_grp2 + anterior_accuracy + posterior_accuracy + anterior_topology + posterior_topology) / 7 + (mean_b0_error + mean_hd95)
