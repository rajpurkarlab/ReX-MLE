"""
Grading script for TopCoW Track 1 Task 3 - CTA Graph Edge Classification
...
METRIC_DIRECTIONS = {
    'anterior_acc': 'higher_better',
    'posterior_acc': 'higher_better',
}
...
def grade(submission: pd.DataFrame, answers: pd.DataFrame, submission_dir: Optional[Path] = None, answers_dir: Optional[Path] = None) -> dict:
    """
    Grade TopCoW Track 1 Task 3 submission using graph classification metrics.
    ...
    if mean_position_value is not None:
        ...
        # Fallback: use negative mean accuracy for ranking
        results_dict['overall'] = -(results_dict['anterior_acc'] + results_dict['posterior_acc']) / 2
