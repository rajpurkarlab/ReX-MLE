"""
Data preparation script for TopCoW Track 1 Task 3 - CTA Graph Edge Classification
...
def prepare(raw: Path, public: Path, private: Path) -> None:
    """
    Prepare TopCoW Track 1 Task 3 data (CTA Classification).

    Expected structure in raw/:
    - imagesTr/: CTA and MRA training images
    - antpos_edges_labelsTr/: Graph edge classification labels
    """
    print(f"Preparing TopCoW Track 1 Task 3 (CTA Classification)...")
    print(f"  Raw: {raw}")
    ...
    for img_path in all_cta_images:
        ...
        # Label files don't have _0000 suffix and are .yml: topcow_ct_001.yml
        label_filename = f"topcow_ct_{case_id}.yml"
    ...
    print(f"Matched {len(matched_cases)} CTA cases with edge classification labels")
    ...
    sample_sub['predicted_edges_path'] = sample_sub['image_id'].apply(
        lambda x: f"predictions/topcow_ct_{x}_edges.json"
    )
    ...
    # Create README
    readme = f"""# TopCoW Track 1 Task 3: CTA Graph Edge Classification
