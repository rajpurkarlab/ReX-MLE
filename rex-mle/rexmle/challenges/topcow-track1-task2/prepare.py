"""
Data preparation script for TopCoW Track 1 Task 2 - CTA 3D Bounding Box Detection
...
def prepare(raw: Path, public: Path, private: Path) -> None:
    """
    Prepare TopCoW Track 1 Task 2 data (CTA Detection).

    Expected structure in raw/:
    - imagesTr/: CTA and MRA training images
    - roi_loc_labelsTr/: 3D bounding box annotations
    """
    print(f"Preparing TopCoW Track 1 Task 2 (CTA Detection)...")
    print(f"  Raw: {raw}")
    ...
    for img_path in all_cta_images:
        ...
        # Label files don't have _0000 suffix and are .txt: topcow_ct_001.txt
        label_filename = f"topcow_ct_{case_id}.txt"
    ...
    print(f"Matched {len(matched_cases)} CTA cases with bounding box labels")
    ...
    for case in test_cases:
        ...
        json_filename = f"topcow_ct_{case['case_id']}_bbox.json"
    ...
    sample_sub['predicted_bbox_path'] = sample_sub['image_id'].apply(
        lambda x: f"predictions/topcow_ct_{x}_bbox.json"
    )
    ...
    # Create README
    readme = f"""# TopCoW Track 1 Task 2: CTA 3D Bounding Box Detection
...
- Labels: `topcow_ct_{{pat_id}}_0000.nii.gz` (binary mask defining bounding box region)
