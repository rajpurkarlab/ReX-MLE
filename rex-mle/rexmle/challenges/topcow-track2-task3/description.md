...
TopCoW Track 2 Task 3 focuses on classifying topological graph edges in the Circle of Willis on MRA (Magnetic Resonance Angiography) imaging. The goal is to accurately classify different edge types in the CoW graph structure.

## Task Description

**Objective:** Classify topological edges in the Circle of Willis graph on MRA images.
...
### Training Data
- **125 MRA cases** with graph edge classification annotations
...
1. **submission.csv** with columns:
   ...
   - `modality`: "MRA"
   - `predicted_edges_path`: Path to prediction JSON file (e.g., "predictions/topcow_mr_001_edges.json")
...
├── submission.csv
└── predictions/
    ├── topcow_mr_001_edges.json
    ├── topcow_mr_002_edges.json
    └── ...
```
