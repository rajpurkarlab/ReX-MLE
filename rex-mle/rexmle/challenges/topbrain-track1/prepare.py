"""
Data preparation script for TopBrain Track 1 - CTA Multiclass Brain Vessel Segmentation

Dataset structure after extraction:
- imagesTr_topbrain_ct/: Training CTA images (topcow_ct_{pat_id}_0000.nii.gz)
- labelsTr_topbrain_ct/: Multiclass segmentation masks with 40 vessel labels
"""
...
def prepare(raw: Path, public: Path, private: Path) -> None:
    """
    Prepare TopBrain Track 1 data (CTA Segmentation).

    Expected structure in raw/:
    - imagesTr_topbrain_ct/: CTA training images
    - labelsTr_topbrain_ct/: Segmentation masks with 40 vessel labels
    """
    print(f"Preparing TopBrain Track 1 (CTA Multiclass Segmentation)...")
    print(f"  Raw: {raw}")
    ...
    shared_images = shared_raw / "imagesTr_topbrain_ct"
    ...
    # Extract if needed
    images_dir = raw / "imagesTr_topbrain_ct"
    labels_dir = raw / "labelsTr_topbrain_ct"
    ...
    if not images_dir.exists() or not labels_dir.exists():
        raise FileNotFoundError(
            f"Expected imagesTr_topbrain_ct/ and labelsTr_topbrain_ct/ directories in {raw}\n"
    ...
    # Create README
    readme = f"""# TopBrain Track 1: CTA Multiclass Brain Vessel Segmentation
...
- **Format**: NIfTI (.nii.gz)
- **Number of Labels**: 40 vessel classes
...
└── labels/        # {len(train_cases)} multiclass brain vessel segmentation masks (40 classes)
...
Segment individual brain vessels into 40 anatomical classes with topology-aware evaluation.
...
2. **predictions/** directory with NIfTI masks:
   - Multiclass masks with vessel labels (0-40)
